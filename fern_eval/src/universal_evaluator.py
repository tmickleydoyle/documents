...
class UniversalCodeEvaluator:
    ...
    def _analyze_language_compatibility(
        ...
        # Calculate overlap percentage as expected by tests (common / max of the
        # two sets)
        max_lang_count = (
        ...
        return {
            ...
            "overlap_percentage": overlap_percentage,  # Add overlap_percentage for
            # tests
            "common_languages": common_languages,  # Keep as set for test compatibility
            "missing_languages": golden_only,  # golden_only_languages renamed
            # to missing_languages
            "extra_languages": generated_only,  # generated_only_languages renamed
            # to extra_languages
    ...
    def _generate_recommendations(
        ...
        if lang_analysis.get("language_similarity", 1.0) < 0.8:
            recommendations.append(
                f"Consider using more similar technologies - language compatibility is "
                f"{lang_analysis.get('language_similarity', 0):.1%}"
            )
        ...
        if missing_functions:
            recommendations.append(
                f"Missing functionality areas: {', '.join(missing_functions[:3])}"
                f"{'...' if len(missing_functions) > 3 else ''}"
            )
        ...
        if low_confidence_count > 0:
            recommendations.append(
                f"Consider improving file organization - {low_confidence_count} files "
                f"had low match confidence"
            )
        ...
        if low_quality_count > 0:
            recommendations.append(
                f"Improve code quality in {low_quality_count} files with low "
                f"similarity scores"
            )
    ...
    def _build_application_report_lines(self, results: Dict[str, Any]) -> List[str]:
        ...
        if "structure_analysis" in results:
            ...
            if func_analysis:
                ...
                # Add covered functionalities if any
                covered = func_analysis.get("covered_functionalities", [])
                if covered:
    ...
    def _resolve_file_path(self, path: str) -> str:
        """Resolve a path to a specific file, handling directories by finding main
        files."""
        path_obj = Path(path)
    ...
    def _evaluation_result_to_dict(self, result) -> Dict[str, Any]:
        """Convert EvaluationResult object to dictionary format for report
        generation."""
        results_dict = {

    ...
    def test_large_application_evaluation_performance(self, benchmark, temp_dir):
        ...
        for component_name, description in components:
            ...
            function {component_name}(props) {{
                ...
                return React.createElement('div', {{className: '{component_name.lower()}'}},
                    React.createElement('h2', null, '{description}'),
                    React.createElement('div', {{className: 'content'}},
                        state && React.createElement('pre', null, JSON.stringify(state, null, 2))
                    ),
                    React.createElement('button', {{onClick: function() {{ handleAction('refresh'); }}}}, 'Refresh')
                );
            }}
...
class TestMemoryUsage:
    ...
    def test_memory_cleanup_batch_processing(self, temp_dir):
        ...
        for i in range(20):
            ...
            # (Adjusted based on realistic usage patterns and ML model memory requirements)

...
class UniversalParser:
    ...
    def parse_application(self, app_path: str) -> ApplicationStructure:
        ...
        try:
            ...
            logger.info(
                f"Parsed {structure.get_primary_language()} application with {structure.total_files()} files"
            )
            return structure
    ...
    def _should_include_file(self, file_path: Path) -> bool:
        ...
        # Check if file extension is supported (include files without extension for config files)
        return (
    ...
    def _detect_language(self, file_path: Path, content: str) -> str:
        ...
        # Return the language with the highest score (minimum 1 keyword for content-based)
        if language_scores:
